...
int checkmap(t_abtmap *game)
{
	...
	while (game->map[i])
	{
		...
		while(game->map[i][j])
		{
			if(game->map[i][j] == 'N' || game->map[i][j] == 'S' || game->map[i][j] == 'E' || game->map[i][j] == 'W' || game->map[i][j] == '0')
            {
                if(!check_valid_map(i, j , game->map, game, &counter))
                    return(0);
            }
            else if (game->map[i][j] != '1' && game->map[i][j] != ' ' && game->map[i][j] != '\n') 
    ...
    if (counter == 0)
        return(printf("map is invalide it should contain 1 player"), exit(1), 0);
    return (1);
}
